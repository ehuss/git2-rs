...
// Create a convenience method on bitflag struct which checks the given flag
...
pub enum ErrorCode {
    ...
    ///
    /// This is a special error that is not generated by libgit2 itself. You
    /// can return it from a callback (e.g. to stop an iteration) to know that
...
/// A listing of the possible states that a repository can be in, based on the
/// current operation which is ongoing.
#[derive(PartialEq, Eq, Clone, Debug, Copy)]
#[allow(missing_docs)]
...
bitflags! {
    ...
    pub struct Sort: u32 {
        ...
        /// Sort the repository contents in topological order (no parents
        /// before all of its children are shown).
        ///
        /// This sorting mode can be combined with time sorting to produce
        /// `git`'s `--date-order``.
...
bitflags! {
    ...
    ///
    /// This is used with a [credentials callback](RemoteCallbacks::credentials).
    pub struct CredentialType: u32 {
        /// A vanilla user/password request.
        ///
        /// See [`Cred::userpass_plaintext`].
        ...
        ///
        /// Used as a pre-authentication step if the underlying transport (e.g.
        /// SSH, with no username in its URL) does not know which username to
        /// use.
        ///
        /// See [`Cred::username`].
...
bitflags! {
    /// Flags for [`IndexEntry::flags_extended`].
    ///
    ...
    /// [`IndexEntry::flags_extended`] value that belong on disk.  You can use
    /// them to interpret the data in the `flags_extended`.
    ///
    /// The rest of the bitmasks match the other fields in the
    /// [`IndexEntry::flags_extended`] value that are only used in-memory by
    /// libgit2. You can use them to interpret the data in the
    /// `flags_extended`.
...
bitflags! {
    ...
    pub struct RepositoryOpenFlags: u32 {
        ...
        /// `start_path`. Do not walk up from the `start_path` looking at
        /// parent directories.
        ...
        /// Unlike [`Repository::open_bare`], this can follow gitlinks.
        const BARE = raw::GIT_REPOSITORY_OPEN_BARE as u32;

        /// Don't try appending `/.git` to the specified repository path.
        ///
        /// Do not check for a repository by appending `/.git` to the
        /// start_path; only open the repository if `start_path` itself points
        ...
        ///
        /// If set, [`Repository::open_ext`] will ignore the other flags and
        /// the `ceiling_dirs` argument. The search for a repository will
        /// respect `$GIT_CEILING_DIRECTORIES` and
        /// `$GIT_DISCOVERY_ACROSS_FILESYSTEM`. The opened repository will
        /// respect `$GIT_INDEX_FILE`, `$GIT_NAMESPACE`,
        /// `$GIT_OBJECT_DIRECTORY`, and `$GIT_ALTERNATE_OBJECT_DIRECTORIES`.
        ///
        /// In the future, this flag will also cause [`Repository::open_ext`]
        /// to respect `$GIT_WORK_TREE` and `$GIT_COMMON_DIR`; currently,
        /// [`Repository::open_ext`] with this flag will error out if either
...
bitflags! {
    /// The results of [`Repository::merge_analysis`] indicating the merge opportunities.
...
impl ObjectType {
    ...
    /// Determine if this is a valid loose object type.
...
impl ConfigLevel {
    /// Converts a [`raw::git_config_level_t`] configuration level to a [`ConfigLevel`].
...
/// The type of change described by a [`DiffDelta`].
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
pub enum Delta {
    /// No changes
    Unmodified,
...
pub enum SubmoduleIgnore {
    ...
    ///
    /// This will not ignore any change - i.e. even an untracked file, will
    /// mark the submodule as dirty. Ignored files are still ignored, of
    /// course.
    None,
    /// Only dirty if tracked files have changed.
    ///
    /// Only changes to tracked files, or the index or the HEAD commit will
    /// matter.
    Untracked,
    /// Only dirty if HEAD has moved.
    ///
    /// Ignores changes in the working directory, only considering changes if
    /// the HEAD of submodule has moved from the value in the superproject.
    Dirty,
    /// Never dirty.
    ///
    /// Never checks if the submodule is dirty.
    All,
}
...
bitflags! {
    ...
    pub struct CheckoutNotificationType: u32 {
        ...
        ///
        /// Dirty files are those that do not need an update but no longer
        /// match the baseline. Core git displays these files when checkout
        /// runs, but won't stop the checkout.
...
bitflags! {
    ...
    ///
    /// When checking attributes, it is possible to check attribute files in
    /// both the working directory (if there is one) and the index (if there
    /// is one). You can explicitly choose where to check and in which order
    /// using the following flags.
    ///
    /// Core git usually checks the working directory then the index, except
    /// during a checkout when it checks the index first. It will use index
    /// only for creating archives or for a bare repo (if an index has been
    /// specified for the bare repo).
    pub struct AttrCheckFlags: u32 {
        ...
        ///
        /// Normally, attribute checks include looking in the `/etc` (or
        /// system equivalent) directory for a `gitattributes` file. Passing
        /// this flag will cause attribute checks to ignore that file.
...
bitflags! {
    ...
    ///
    /// These flags are used for both [`DiffDelta::flags`] and
    /// [`raw::git_diff_file::flags`] representing the old and new sides of
    /// the delta. Values outside of this public range should be considered
